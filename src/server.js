...
function makeTransport({ port, secure }) {
  return nodemailer.createTransport({
    ...
    socketTimeout: 10000,
    // If your SMTP uses self-signed certs behind a proxy:
...
/** 1) Security headers (no CSP to allow inline on simple pages) */
app.use(
  helmet({
...
/** 2.1) ✅ Safe rate limiter (default key generator; proxy set to 1) */
...
const leadSchema = Joi.object({
  ...
  toestemming: Joi.boolean().truthy('on').falsy('off').default(false), // ✔ not required
...
app.post('/leads', async (req, res) => {
  try {
    ...
    // Mail naar praktijk — run async, met retry
    if (praktijk_code) {
...
${baseUrl}/lead-action?action=afspraak_gemaakt&lead_id=${inserted.id}&practice_code=${practice.code}&token=${actionToken}
        ...
        // Send in background; don’t delay response
        (async () => {
          try {
...
/** 11) Lead-action confirmation page (unchanged logic) */
app.get('/lead-action', async (req, res) => {
  try {
    ...
    res.send(`<h2>✅ Actie geregistreerd voor ${updated.lead.volledige_naam}</h2>`);
  } catch (error) {
